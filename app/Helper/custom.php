...
define('PRODUCT_AVATAR_PATH','product/');
define('HERO_AVATAR_PATH','hero/');
define('ABOUT_AVATAR_PATH','about/');
define('SPECIAL_AVATAR_PATH','special/');
define('EVENT_AVATAR_PATH','event/');
...
if (!function_exists('availble_status')) {
    function availble_status(int $id,int $status,string $name = null){
        return $status == 1 ? '<span class="badge badge-success availble_status" data-id="' . $id . '" data-name="' . $name . '" data-status="2" style="cursor:pointer;">In Stock</span>' :
        '<span class="badge badge-danger availble_status" data-id="' . $id . '" data-name="' . $name . '" data-status="1" style="cursor:pointer;">Stock Out</span>';
    }
}
