...
class CartController extends Controller
{
    ...
    public function store_or_update($request){
        ...
        $collection = $collection->merge(compact('created_by','created_at','user_id','product_id','product_qty'));
        return Cart::updateOrCreate($collection->all());
    }
    ...
    public function updateOrStore(CartRequest $request){
        if ($request->ajax()) {
    ...
    //     if (Auth::check()) {
    //         $prod_check = Cart::where('id',$product_id)->exists();
    //         if($prod_check){
    ...
    public function bulkDelete(Request $request){
        if ($request->ajax()) {
            $result = Cart::whereIn('id',$request->ids)->select('image')->get();
            if($result){
