...
class ProductController extends Controller
{
    ...
    public function store_or_update($request){
        ...
        $section_name = 'ProductSection';
        $image = $request->old_image;
        if($request->hasFile('image')){
            $image = $this->upload_file($request->file('image'),PRODUCT_AVATAR_PATH);
            if(!empty($request->old_image)){
                $this->delete_file($request->old_image,PRODUCT_AVATAR_PATH);
            }
        }
    ...
    public function index(Request $request){
        ...
        if($request->ajax()){
            $getData = Product::orderBy('id','desc');
            return DataTables::eloquent($getData)
                ...
                ->addColumn('available', function($row){
                    return STOCK[$row->available] ?? 'N/A';
                })
                ...
                ->addColumn('image', function($row){
                    if ($row->image != null) {
                        $attach = '<img src="'.url('uploads/'.PRODUCT_AVATAR_PATH.$row->image).'" width="50px">';
                    }else{
                        $attach = '';
                    }
                    return $attach;
                ...
                ->rawColumns(['bulk_check','image','available','status','action'])
                ->make(true);
        }
        ...
        return view('backend.product.index', ['breadcrumb'=>$breadcrumb, 'category'=>$category]);
    }
    ...
    public function bulkDelete(Request $request){
        if ($request->ajax()) {
            $result = Product::whereIn('id',$request->ids)->select('image')->get();
            if($result){
                foreach ($result as $value) {
                    $this->delete_file($value->image,PRODUCT_AVATAR_PATH);
                }
