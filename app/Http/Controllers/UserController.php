...
class UserController extends Controller
{
    ...
    public function index(Request $request){
        if($request->ajax()){
            ...
            return DataTables::eloquent($getData)
                ...
                ->addColumn('image', function($row){
                    if ($row->image != null) {
                        $attach = '<img src="'.url('uploads/'.USER_AVATAR_PATH.$row->image).'" width="50px">';
                    }else{
                        $attach = '';
                    }
                    return $attach;
        ...
        $this->setPageTitle('User','User List');
        $breadcrumb = ['User'=>''];
    ...
    public function updateOrStore(UserRequest $request){
        if ($request->ajax()) {
