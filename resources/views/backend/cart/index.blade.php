        ...
        <div class="box box-solid">
            <div class="box-header with-border">
                <div class="box-tools pull-right">
                    {{-- <a href="{{ route('app.cart.import') }}" class="btn btn-sm btn-info rounded-0"><i class="fa fa-upload fa-sm"></i> Import</a> --}}
                    <button type="button" class="btn btn-sm btn-primary rounded-0" onclick="showFormModal('New Cart', 'Save')"><i class="fa fa-plus fa-sm"></i> Add Cart</button>
                </div>
            </div>
            <div class="box-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover table-striped" id="branch-datatable" style="width: 100%">
                        <thead>
                            <th>
...
@include('backend.cart.update_or_store')
@endsection
@push('scripts')
    <script>
        ...
        $(document).on('click', '#save-btn', function () {
            ...
            let url = "{{ route('app.cart.update-or-store') }}";
        ...
        $(document).on('click', '.edit_data', function () {
            ...
            if (id) {
                $.ajax({
                    url: "{{ route('app.cart.edit') }}",
                    type: "POST",
                    data: { id: id,_token: _token},
                    dataType: "JSON",
                    success: function (data) {
                        $('#store_or_update_form #update_id').val(data.data.id);
                        $('#store_or_update_form #user_id').val(data.data.user_id);
                        $('#store_or_update_form #product_id').val(data.data.product_id);
                        $('#store_or_update_form #product_qty').val(data.data.product_qty);
        ...
        $(document).on('click','.change_status', function(){
            ...
            var url = "{{ route('app.cart.status-change') }}"
        ...
        $(document).on('click', '.delete_data', function () {
            let id = $(this).data('id');
            let title = $(this).data('product_id');
            let row = table.row($(this).parent('tr'));
            let url = "{{ route('app.cart.delete') }}";
            delete_data(id,url,row,product_id);
        });
        ...
        function multi_delete(){
            ...
            if(ids.length == 0){
                ...
                let url = "{{ route('app.cart.bulk-delete') }}";
