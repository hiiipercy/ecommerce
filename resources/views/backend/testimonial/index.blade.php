        ...
        <div class="box box-solid">
            <div class="box-header with-border">
                <div class="box-tools pull-right">
                    {{-- <a href="{{ route('app.testimonial.import') }}" class="btn btn-sm btn-info rounded-0"><i class="fa fa-upload fa-sm"></i> Import</a> --}}
                    <button type="button" class="btn btn-sm btn-primary rounded-0" onclick="showFormModal('New Testimonial', 'Save')"><i class="fa fa-plus fa-sm"></i> Add Testimonial</button>
                </div>
            </div>
            <div class="box-body">
...
@include('backend.testimonial.update_or_store')
@endsection
@push('scripts')
    <script>
        table = $('#branch-datatable').DataTable({
            ...
            ajax: {
                url: "{{ route('app.testimonial.index') }}",
                type: "GET",
                dataType: "JSON",
                data: function(d) {
                    d._token = _token;
        ...
        $(document).on('click', '#save-btn', function () {
            ...
            let url = "{{ route('app.testimonial.update-or-store') }}";
        ...
        $(document).on('click', '.edit_data', function () {
            ...
            if (id) {
                $.ajax({
                    url: "{{ route('app.testimonial.edit') }}",
                    type: "POST",
                    ...
                    success: function (data) {
                        ...
                        $('#store_or_update_form #profession').val(data.data.profession);
                        $('#store_or_update_form #description').val(data.data.description);
                        $('#store_or_update_form #star').val(data.data.star);
        ...
        $(document).on('click', '.delete_data', function () {
            ...
            let url = "{{ route('app.testimonial.delete') }}";
        ...
        function multi_delete(){
            ...
            if(ids.length == 0){
                ...
                let url = "{{ route('app.testimonial.bulk-delete') }}";
